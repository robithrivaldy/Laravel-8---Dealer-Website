...
@section('content')
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 ">
        <div
            ...
            <h1 class="h2">Edit Brand {{ $item['brand'] }} </h1>
        </div>
        <form action="{{ url('brand/update/' . $item['id']) }}" method="POST" enctype="multipart/form-data">

            @csrf
            <div class="form-group">
                <label for="exampleFormControlInput1">Brand Name</label>
                <input type="text" name="brand" class="form-control" value="{{ $item['brand'] }}"
                    placeholder="Ex. Toyota Honda Lexus BMW" required>
...
@push('after-script')
    ...
    @if ($item['image'] != '')
        <script>
            $(document).ready(function() {
                $("#image1").attr("src", "{{ url($item['image']) }}");
                $("#deleteimage1").show();
            });
